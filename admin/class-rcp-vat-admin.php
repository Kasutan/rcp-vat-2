...
class Rcp_Vat_Admin
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        */
        ...
        //Store basic payment info to Payment Meta Table
        add_action('rcp_create_payment', array(
            $this,
            'rcp_vat_save_payment_meta_data_1'
        ), 10, 2);
        //Store vat info to Payment Meta Table
        add_action('rcp_update_payment', array(
            $this,
            'rcp_vat_save_payment_meta_data_2'
        ), 10, 2);

        //Add VAT info to payments page screen in admin
        add_action('rcp_payments_page_table_header', array(
            $this,
            'rcp_vat_payments_page_table_header'
        ));
        
        add_action('rcp_payments_page_table_column', array(
            $this,
            'rcp_vat_payments_page_table_column'
        ), 10, 1);
    }
    ...
    function rcp_vat_get_vat_rate()
    {
        ...
        if ($vat_business_number) {
            ...
            try {
                if (! $validator->validateFormat($vat_business_number)) {
                    ...
                    $valid_vat_number = 1;
                    $vat_label=__('Valid intra-community number - no VAT','rcp-vat');
                    $country_vat_rate = 0;
                }
            } catch (Exception $e) {
        ...
        $result = array(
            ...
            'valid_vat_number' => $valid_vat_number,
        );
    ...
    function needToApplyVAT($user_id)
    {
        ...
        $countries = new DvK\Vat\Countries();
        $validator = new DvK\Vat\Validator();
        $billing_country = $this->getUserCountry($user_id); // [billing_country] => FR
    ...
    function modify_rcp_stripe_create_subscription_args($sub_args, $obj)
    {
        ...
        if ($userWP) {
            ...
            $billing_city = get_user_meta($userWP->ID, 'billing_city', true);
            ...
            $sub_args["metadata"]['billing_city'] = $billing_city;
    ...
    {
        global $rcp_options;
        /*rcp_log('### modify_rcp_subscription_data before sending to gateway...');
        rcp_log('Valeurs avant modif: ');
        rcp_log('price: '.$subscription_data["price"]);
        rcp_log('fee: '.$subscription_data["fee"]);
        rcp_log('discount: '.$subscription_data["discount"]);*/
        ...
        if ($enable && $vat_rate !== false) {
            ...
            $subscription_data["fee"] = $this->getTotalPrice($subscription_data["fee"], $vat_rate);
            $subscription_data["discount"] = $this->getTotalPrice($subscription_data["discount"], $vat_rate);
            $subscription_data["recurring_price"] = $this->getTotalPrice($subscription_data["recurring_price"], $vat_rate);
        }
        
        /*
        rcp_log('Valeurs après modif: ');
        rcp_log('price: '.$subscription_data["price"]);
        rcp_log('fee: '.$subscription_data["fee"]);
        rcp_log('discount: '.$subscription_data["discount"]);*/
...
<p>
    ...
    <span><em><?php _e('Please enter the whole number including country code but excluding spaces.','rcp-vat');?></em></span>
</p>
...
<p>
	<label for="billing_city"><?php _e( 'City', 'rcp-vat' ); ?></label>
...
<?php
    ...
    {
        rcp_log('### VALIDATE ### rcp_vat_rcp_validate_user_fields_on_register');
        $vatStart = '';
        ...
        if ($this->use_wc_billing_fields) {
            ...
            if (empty($posted['billing_city'])) {
                rcp_errors()->add('invalid_city', __('Please enter your city', 'rcp-vat'), $dest_form);
            }
            if (empty($posted['billing_postcode'])) {
            ...
            if (empty($posted['billing_country'])) {
                ...
                $msg = __('VAT Number and Country mismatch. Please make sure that you entered your VAT number including country code.', 'rcp-vat') . '&nbsp;' . $vatStart . ' / ' . $posted['billing_country'];
...
<table class="form-table">
    ...
    /**
     * Save basic info to payment meta data table when payment is created
     */
    public function rcp_vat_save_payment_meta_data_1($payment_id,$payment_data) {
       

        global $rcp_payments_db;
        $amount_ht=$payment_data['amount'];
        $user_id=$payment_data['user_id'];
        
        // Add metadata
        $rcp_payments_db->update_meta( $payment_id, $key = 'amount_no_tax', $value = $amount_ht, $unique = true );
        $rcp_payments_db->update_meta( $payment_id, $key = 'user_id', $value = $user_id, $unique = true );
        
        //rcp_log('Montant HT ajouté : '.$amount_ht);
    }
    /**
     * Save vat info to payment meta data table when payment is updated (by then user meta is available)
     */
    public function rcp_vat_save_payment_meta_data_2($payment_id,$payment_data) {
        //rcp_log('payment mis à jour : '.$payment_id );
      

        global $rcp_payments_db;
        $user_id=$rcp_payments_db->get_meta( $payment_id, $key = 'user_id',  $unique = true );
        $amount=$rcp_payments_db->get_meta( $payment_id, $key = 'amount_no_tax',  $unique = true );
        
        //rcp_log('Retrieve user id: '.$user_id);
        //rcp_log('Retrieve amount ht: '.$amount);

        $vat_rate=$this->needToApplyVAT($user_id);      
        $vat_amount=$this->getVATAmount($amount,$vat_rate);
        $ttc_amount=$this->getTotalPrice($amount,$vat_rate);
        $user_country=$this->getUserCountry($user_id);
        //rcp_log('User country: '.$user_country);
        
        // Add metadata
        $rcp_payments_db->update_meta( $payment_id, $key = 'vat_rate', $value = $vat_rate, $unique = true );
        $rcp_payments_db->update_meta( $payment_id, $key = 'vat_amount', $value = $vat_amount, $unique = true );
        $rcp_payments_db->update_meta( $payment_id, $key = 'amount_tax_inc', $value = $ttc_amount, $unique = true );
        $rcp_payments_db->update_meta( $payment_id, $key = 'user_country', $value = $user_country, $unique = true );
        
        //rcp_log('Meta data added');
    }

    /**
     * Display vat info in payments admin screen.
     */
    public function rcp_vat_payments_page_table_header() {
        ?>
        <th scope="col" class="rcp-payments-vat-rate-col"><?php _e( 'TVA rate', 'rcp' ); ?></th>
        <th scope="col" class="rcp-payments-vat-amount-col"><?php _e( 'TVA amount', 'rcp' ); ?></th>
        <th scope="col" class="rcp-payments-ttc-amount-col"><?php _e( 'TTC amount', 'rcp' ); ?></th>
        <?php
    }

    public function rcp_vat_payments_page_table_column($payment_id) {
        global $rcp_payments_db;
        ?>
        <td data-colname="<?php _e( 'VAT rate', 'rcp' ); ?>">
            <?php echo $rcp_payments_db->get_meta( $payment_id, $key = 'vat_rate', $unique = true );?>
        </td>
        <td data-colname="<?php _e( 'VAT amount', 'rcp' ); ?>">
            <?php echo $rcp_payments_db->get_meta( $payment_id, $key = 'vat_amount', $unique = true ); ?>
        </td>
        <td data-colname="<?php _e( 'TTC amount', 'rcp' ); ?>">
            <?php echo $rcp_payments_db->get_meta( $payment_id, $key = 'amount_tax_inc', $unique = true );?>
        </td>
        <?php
    }
}